...
class FindParamsMod(Mod):
    ...
    async def request_with_sem(self, url, params):
        async with sem:
            return await self.requeser.request("GET", url, params=params)

    async def crack_params(self, page: Page, example_resp, params):
        for payload in payloads:
            resp = await self.request_with_sem(
                page.url, params={p: payload for p in params}
            )
            if resp.text == example_resp.text:
                continue

            results = await asyncio.gather(
                *[self.request_with_sem(page.url, params={p: payload}) for p in params]
            )
            respond_params = [
